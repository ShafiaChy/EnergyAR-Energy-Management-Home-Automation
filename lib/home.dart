...
class _HomeState extends State<Home> {
  ...
  void setWatt() {
    showDialog(
      context: context,
      builder: (context) {
        var refWatt = TextEditingController();
        var fanWatt = TextEditingController();
        var lightWatt = TextEditingController();
        return AlertDialog(
          title: const Text("Set Watt"),
          scrollable: true,
          content: Column(
            children: [
              TextField(
                ...
                decoration: const InputDecoration(hintText: "Fridge Watt", labelText: "Fridge"),
              ),
              TextField(
                controller: fanWatt,
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(hintText: "Fan Watt", labelText: "Fan"),
              ),
              TextField(
                controller: lightWatt,
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(hintText: "Light Watt", labelText: "Light"),
              ),
              const SizedBox(height: 15),
              FilledButton(
                  onPressed: () {
                    if (client.isConnected()) {
                      client.sendMessage("WT=${refWatt.text},${fanWatt.text},${lightWatt.text},");
                      snackBar("Watt updated.", Colors.blueAccent, Colors.white);
                    }
                      ...
                      child: CameraPreview(
                        ...
                        child: Stack(
                          children: [
                            ...
                            if (!webMode)
                              Positioned(
                                ...
                                child: FilledButton(
                                  style: ButtonStyle(
                                    backgroundColor:
                                        isConnected ? MaterialStateProperty.all(Colors.green) : MaterialStateProperty.all(Colors.red),
                                  ),
                            ...
                            //     child: FilledButton(
                            //       onPressed: setWatt,
                            //       child: const Text("Set\nWatt", textAlign: TextAlign.center),
                            //     ),
                            //   ),
                          ],
                        ),
